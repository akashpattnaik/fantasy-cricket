...
players_mat = np.zeros((len(match_info["match_ids"]), len(players_df.index)))
...
for ind, match_id in enumerate(match_info["match_ids"]):
    players_mat[ind,:] = 0
    ...
    for index, row in batting_data.iterrows():
        ...
        players_mat[ind, comm_names.index(name)] += point_change
        ...
        if dismissal.find("sub (") == -1:
            ...
            if dismissal.find("st") == 0:
                ...
                fielder = re.sub(r"\W+", ' ', fielder).strip()
                fielder_com_name = [name for name in xis if fielder in name]
                players_mat[ind, comm_names.index(fielder_com_name[0])] += points["stumping"]
            # run out
    ...
    for player in xis:
        players_mat[ind, comm_names.index(player)] += points["in_xi"]


update_ipl_sheet("IPL Squads - Week " + str(match_info["week"]), players_mat)